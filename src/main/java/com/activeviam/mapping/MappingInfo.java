...
public class MappingInfo {
	...
	/**
	 * Metadata information about a mapping generated by {@link MappingGenerator}, specific to a module.
	 *
	 * @author ActiveViam
	 */
	...
	@Override
	public String toString() {
		final StringBuilder sb = new StringBuilder("MAPPING SUMMARY").append(MigrationUtils.LINE_SEPARATOR)
				...
				.append(MigrationUtils.LINE_SEPARATOR);
		this.moduleInfos.values()
				.stream()
				.map(ModuleInfo::toString)
				.forEach(moduleInfo -> sb.append("\t").append(moduleInfo).append(MigrationUtils.LINE_SEPARATOR));
		return sb.toString();
	}

	private int getTotalMappingSize() {
		return this.moduleInfos.values().stream().mapToInt(moduleInfo -> moduleInfo.impactedFiles).sum();
	}
